...
<div class="outer-box-container">
	...
	<img src="images/dms-banner.jpg" class="banner">
		...
		<div class="operation">
		<s:form action="department_sharedocument" method="post">
		 <s:hidden name="documentid" value="%{documentid}" /> 		
		<s:actionmessage/>
		<s:actionerror/>
		<s:label value="Sharing %{documentname}"/>
		<s:actionerror/>
		<s:select label="User" headerKey="-1"
			headerValue="Select User" list="users"
			name="curr_user" />		
		<s:actionerror/>
		<s:checkboxlist label="Privilege(s)" list="privileges" name="curr_privilege" />
		<s:submit value="Share" align="center" />
		</s:form>
		</div>
	</div>
	</div>
